...
static inline int64_t _initial_scramble(int64_t s)
{
    return (s ^ JRAND_MULTIPLIER) & JRAND_MASK;
}

static inline int64_t _system_time()
{
...
static inline int64_t _seed_uniquifier()
{
    return (_SEED_UNIQUIFIER = _SEED_UNIQUIFIER * 181783497276652981L);
}

static inline void jrand_init_seed(jrand_t *j, int64_t s)
{
    j->state = _initial_scramble(s);
    j->has_g = false;
}

static inline void jrand_init(jrand_t *j)
{
    jrand_init_seed(j, _seed_uniquifier() ^ _system_time());
}

static inline void jrand_set_seed(jrand_t *j, int64_t s)
{
    j->state = _initial_scramble(s);
    j->has_g = false;
}
...
static inline int32_t _jrand_next(jrand_t *j, size_t b)
{
...
static inline void jrand_next_bytes(jrand_t *j, int8_t *arr, size_t len)
{
...
static inline int32_t jrand_next_int(jrand_t *j)
{
    return _jrand_next(j,32);
}

// requires bound 0 < b < 2^31 (not checked)
static inline int32_t jrand_next_int_mod(jrand_t *j, int32_t b)
{
...
static inline int64_t jrand_next_long(jrand_t *j)
{
...
static inline bool jrand_next_bool(jrand_t *j)
{
    return _jrand_next(j,1);
}

static inline float jrand_next_float(jrand_t *j)
{
    return _jrand_next(j,24) / (float) 0x1000000;
}

static inline double jrand_next_double(jrand_t *j)
{
...
static inline double jrand_next_gaussian(jrand_t *j)
{
    if (j->has_g)
    {
        double ret = j->next_g;
